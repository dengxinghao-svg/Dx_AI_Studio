(function () {
  ...
  const messages = {
    "zh-CN": {
      ...
      blankPanel: "独立空白面板",
      justCreated: "刚刚创建",
      editedMinutes: (n) => `编辑于 ${n} 分钟前`,
      editedHours: (n) => `编辑于 ${n} 小时前`,
      editedDays: (n) => `编辑于 ${n} 天前`,
      open: "打开",
      rename: "重命名",
      move: "移动至...",
      delete: "删除",
    ...
    en: {
      ...
      editedMinutes: (n) => `Edited ${n} min ago`,
      editedHours: (n) => `Edited ${n} hr ago`,
      editedDays: (n) => `Edited ${n} day ago`,
      open: "Open",
      rename: "Rename",
      move: "Move to...",
      delete: "Delete",
  ...
  function formatUpdatedAt(timestamp) {
    ...
    if (minutes < 1) return t().justCreated;
    if (minutes < 60) return t().editedMinutes(minutes);
    if (hours < 24) return t().editedHours(hours);
    return t().editedDays(days);
  }
    ...
    store.listProjects().forEach((project) => {
      ...
      card.innerHTML = `
        ...
        <div class="project-card__content">
          ...
          <p class="project-card__meta ${project.snapshot ? "" : "project-card__meta--muted"}">${project.snapshot ? formatUpdatedAt(project.updatedAt) : t().blankPanel}</p>
        </div>
      `;
